...
async function sendToTelegram(message: string) {
  ...
  const chatId = getEnv("TELEGRAM_CHAT_ID_IPBEZBANK");
...
async function persistLeadToFile(payload: RelayPayload) {
  ...
  const filePath = path.join(storageDir, "quiz-ipbezbank-leads.jsonl");
...
async function sendToRelayWebhook(payload: RelayPayload) {
  ...
  const chatId = getOptionalEnv("TELEGRAM_CHAT_ID_IPBEZBANK") || "";
...
export async function POST(req: Request) {
  try {
    ...
    const message = [
      "<b>Новая заявка: квиз ипотека (ipbezbank)</b>",
      "",
    ...
    try {
      ...
      console.error("Quiz ipbezbank telegram delivery error:", telegramError);
    }

    const fallbackFilePath = await persistLeadToFile(relayPayload);
    console.warn("Quiz ipbezbank lead saved to local file fallback:", fallbackFilePath);

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("Quiz ipbezbank submit error:", error);
    return NextResponse.json(
